    ...
    : AudioProcessorEditor (&p), audioProcessor (p)
{
    ...
    licenseBox.setBounds(0, 0, getWidth(), 20);
    licenseBox.setTopLeftPosition(0, getHeight() / 2 + 25);
    juce::Logger::outputDebugString(juce::String(p.activationId));
    licenseBox.setText(juce::String(p.activationId));
...
void OddballtheAlchemystAudioProcessorEditor::paint (juce::Graphics& g)
{
    ...
    // If activation status is true render normaly
    if (!audioProcessor.activationStatus) { // Display id to user register plugin
        g.drawText(juce::String("Thats your activation id: \n"), juce::Rectangle<float>(0, getHeight() * 0.5, getWidth(), 20), juce::Justification::centred, true);
    }
